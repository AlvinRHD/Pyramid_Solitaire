...
namespace SolitarioPiramide.Game
{
    public static class GameManager
    {
        ...
        private static void DealCards()
        {
            for (int row = 0; row < 7; row++)
            {
                ...
                for (int col = 0; col < row + 1; col++)
                {
                    ...
                    pyramid[row][col].IsFaceUp = (row == 6);
                }
            }
        ...
        private static void PlayGame()
        {
            ...
            while (gameRunning)
            {
                ...
                var input = AnsiConsole.Ask<string>("Seleccione dos cartas que sumen 13 (formato: fila1,col1 fila2,col2), " +
                    ...
                    "'w' para usar la carta seleccionada del stock y si la 'k' sale en el stock se elimina automáticamente al seleccionar w" +
                    ", 'k' para eliminar un Rey de la pirámide:");
                ...
                if (input.ToLower() == "s")
                {
                    ...
                    if (waste.Count > 0)
                    {
                        stockCard = waste[waste.Count - 1];
                        Console.WriteLine($"StockCards después de lanzarlas: {stockCard.ToString()}");
                    }
                    else
                    {
                        stockCard = null;
        ...
        private static bool IsGameOver()
        {
            ...
            if (waste.Count > 0)
            {
                Card stockCard = waste[waste.Count - 1];
                if (stockCard.Value == 13)
                {
                    return false;
                }
                ...
                for (int row = 0; row < 7; row++)
                {
                    for (int col = 0; col < row + 1; col++)
                    {
                        if (pyramid[row][col] != null && pyramid[row][col].IsFaceUp)
                        {
                            if (stockCard.Value + pyramid[row][col].Value == 13)
                            {
                                return false;
                            }
                        }
...
""");
            ...
            {
                                                   Console.Write("│");
                string rowStr = new string(' ', (7 - row) * 3);
                for (int col = 0; col < row + 1; col++)
                {
                    rowStr += "[" + (pyramid[row][col] != null ? EscapeMarkup(pyramid[row][col].ToString()) : "  ") + "] ";
                }
                                
                         Console.WriteLine($"{rowStr.PadRight(70)}    │");
            }
